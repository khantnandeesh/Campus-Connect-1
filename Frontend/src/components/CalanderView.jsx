...
const MeetingCalendar = ({ user }) => {
  ...
  useEffect(() => {
    const fetchMeetings = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get("http://localhost:3000/api/meetings/all", {
  ...
  return (
    <div className="calendar-container" style={{ minHeight: "100vh", backgroundColor: "#121212", color: "#e0e0e0", display: "flex", flexDirection: "column", alignItems: "center", padding: "24px" }}>
      <div className="calendar-wrapper" style={{ width: "100%", maxWidth: "1000px", backgroundColor: "#1e1e1e", boxShadow: "0px 4px 20px rgba(0,0,0,0.3)", borderRadius: "8px", padding: "24px" }}>
        <div className="header" style={{ textAlign: "center", marginBottom: "24px" }}>
          <h1 style={{ fontSize: "26px", fontWeight: "600", color: "#e0e0e0", marginBottom: "8px" }}>Meeting Dashboard</h1>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #333", paddingBottom: "16px" }}>
            <div style={{ textAlign: "left" }}>
              <p style={{ margin: "4px 0", fontSize: "14px" }}>
                <span style={{ color: "#a0a0a0" }}>Welcome,</span> <strong>{user?.username || "User"}</strong>
              </p>
              <p style={{ margin: "4px 0", fontSize: "14px" }}>
                <span style={{ color: "#a0a0a0" }}>Email:</span> <strong>{user?.email}</strong>
              </p>
            </div>
            <div style={{ display: "flex", gap: "12px" }}>
              <button
                ...
                onClick={() => navigate(`/find-users/${user?.id}`)}
              >
                Find Users
              </button>
              <button
          ...
          <div className="calendar-section">
            <FullCalendar
              ...
              viewDidMount={(view) => {
                ...
                Object.entries(calendarCustomStyles).forEach(([selector, styles]) => {
                ...
                if (calendarElement) {
                  calendarElement.style.fontFamily = "'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif";
      ...
      {selectedMeeting && (
        <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
          <div style={{ backgroundColor: "#1e1e1e", color: "#e0e0e0", padding: "24px", borderRadius: "8px", boxShadow: "0px 4px 25px rgba(0,0,0,0.4)", width: "380px", position: "relative" }}>
            <button 
            ...
            <h3 style={{ fontSize: "20px", fontWeight: "600", marginBottom: "16px", paddingBottom: "8px", borderBottom: "1px solid #333" }}>
              {selectedMeeting.title}
            </h3>
            <div style={{ marginBottom: "20px" }}>
              <p style={{ marginBottom: "12px", fontStyle: "italic", color: "#a0a0a0" }}>{selectedMeeting.description}</p>
              <p style={{ marginBottom: "8px", fontSize: "14px" }}>
                <strong style={{ color: "#bdbdbd" }}>Date & Time:</strong>{" "}
                <span>{new Date(selectedMeeting.date).toLocaleString(undefined, {
                  weekday: 'short',
                  year: 'numeric',
                  month: 'short',
                  day: 'numeric',
              ...
              <p style={{ marginBottom: "12px", fontSize: "14px" }}>
                ...
                <span style={{
                  ...
                  fontSize: "12px",
                  fontWeight: "500",
                  backgroundColor: `${getStatusColor(selectedMeeting.status, 0.2)}`,
                  color: getStatusColor(selectedMeeting.status, 1),
                  border: `1px solid ${getStatusColor(selectedMeeting.status, 0.4)}`
                }}>
            ...
            <div style={{ marginBottom: "20px", padding: "12px", backgroundColor: "#252525", borderRadius: "6px", fontSize: "14px" }}>
              {message.length > 0 ? (
                <div style={{ textAlign: "center" }}>
                  <p style={{ marginBottom: "8px", fontWeight: "500" }}>Meeting Room Code</p>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
                    ...
                    <Tooltip id="copy-tooltip" content="Copy to clipboard" place="top" />
                  </div>
                </div>
              ) : (
                <p style={{ textAlign: "center", color: "#a0a0a0" }}>Room code will be available on the meeting date</p>
              )}
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", marginBottom: "16px" }}>
              <button
                ...
                onClick={() => {
                  let win = window.open(`https://konnect-fun.vercel.app`, '_blank');
                  win.focus();
                }}
              >
